...
public class CardDeliveryTest {
    ...
    void positiveTest() {
        ...
        UserData user = UserGenerator.generateUser(5);
        ...
        $("[data-test-id='date'] .input__control").setValue(user.getDate());
        $("[data-test-id='name'] input").setValue(user.getName());
        $("[data-test-id='phone'] input").setValue(user.getPhoneNumber());
        $("[data-test-id='agreement'] span").click();
        $x("//span[text()='Запланировать']").click();
        $(".notification__content")
                .shouldHave(Condition.text("Встреча успешно запланирована на " + user.getDate().toString()), Duration.ofSeconds(15))
        ...
        $("[data-test-id='date'] .input__control").sendKeys(Keys.CONTROL + "A");
        $("[data-test-id='date'] .input__control").sendKeys(BACK_SPACE);
        $("[data-test-id='date'] .input__control").setValue(UserGenerator.generateDate(6));
        $x("//span[text()='Запланировать']").click();
        ...
        $(".notification__content")
                .shouldHave(Condition.text("Встреча успешно запланирована на " + UserGenerator.generateDate(6).toString()), Duration.ofSeconds(15))
                .shouldBe(Condition.visible);
    }
}
